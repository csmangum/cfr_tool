...
def check_sqlite_directly(db_path: str):
    ...
    try:
        ...
        if sample:
            ...
            logger.info(
                f"  Embedding size: {len(sample[6]) if sample[6] else 'None'} bytes"
            )
...
def check_sqlalchemy_connection(db_url: str):
    ...
    try:
        ...
        # Check sample
        sample = session.query(BaseRegulationChunk).first()
        if sample:
            logger.info("Sample row found via SQLAlchemy:")
            logger.info(f"  ID: {sample.id}")
            logger.info(f"  Agency: {sample.agency}")
            logger.info(
                f"  Embedding size: {len(sample.embedding) if sample.embedding else 'None'} bytes"
            )
